...
func TestUninstallImpl(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			uninstaller := NewUninstallerImpl(c, &lister)
			require.NoError(t, uninstaller.Uninstall(context.Background(), tc.CSVNamespace, tc.CSVPrefix))

			assert.Error(t, c.Get(
				context.Background(),
				client.ObjectKeyFromObject(&tc.ActualCSV),
				new(opsv1alpha1.ClusterServiceVersion),
			),
			)
		})
	}
...
type csvListerMock struct {
	mock.Mock
}

func (m *csvListerMock) ListCSVs(ctx context.Context, opts ...ListCSVsOption) ([]opsv1alpha1.ClusterServiceVersion, error) {
...
func TestCSVListerImpl(t *testing.T) {
	t.Parallel()
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			lister := NewCSVListerImpl(client)
